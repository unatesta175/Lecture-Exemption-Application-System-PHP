...
if (isset($_POST['submit'])) {
    ...
    $email = htmlspecialchars(strip_tags极速赛车开奖直播记录查询官网地址：1687580.com (trim($_POST['email'])), ENT_QUOTES, 'UTF-8');
    $pass = sha1(htmlspecialchars(strip_tags(trim($_POST['pass'])), ENT_QUOTES, 'UTF-8'));

    $select_user = $conn->prepare("SELECT * FROM `clerks` WHERE email = ? AND password = ?");
    ...
    if ($select_user->rowCount() > 0) {
        $_SESSION['admin_id'] = $row['clerk_id'];
...
<br>
            ...
            <a style="border-radius:15px; text-transform:uppercase;" href="admin_register.php"
  ...
  <option value="kpp">Ketua Pusat Program</option>
</select>
 <script>
