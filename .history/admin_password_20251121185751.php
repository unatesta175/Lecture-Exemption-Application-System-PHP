...
if (isset($_POST['submit'])) {
    ...
    $fetch_pass_stmt = $conn->prepare("SELECT password FROM clerks WHERE clerk_id = ?");
    $fetch_pass_stmt->execute([$admin_id]);
    ...
    if ($old_pass == $empty_pass) {
        ...
        $update_stmt = $conn->prepare("UPDATE `clerks` SET password = ? WHERE clerk_id = ?");
        $update_stmt->execute([$new_pass, $admin_id]);
