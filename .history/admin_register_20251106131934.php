...
if (isset($_POST['submit'])) {
   ...
   $admin_name = htmlspecialchars(strip_tags(trim($_POST['admin_name'])), ENT_QUOTES, 'UTF-8');
   ...
   $cpassword = htmlspecialchars(strip_tags(trim($_POST['cpassword'])), ENT_QUOTES, 'UTF-8');
   ...
   $select_user = $conn->prepare("SELECT * FROM `clerks` WHERE email = ?");
   $select_user->execute([$email,]);
   $row = $select_user->fetch(PDO::FETCH_ASSOC);
   ...
   if ($select_user->rowCount() > 0) {
      ...
      if ($password != $cpassword) {
         ...
         $insert_user = $conn->prepare("INSERT INTO `clerks`(clerk_name , staff_no , email, password , work_phone_no) VALUES(?,?,?,?,?)");
         $insert_user->execute([$admin_name, $staff_no, $email, $password, $work_phone_no ]);
...
<body>
   ...
   <section class="form-container" styles="border-radius: 15px;">
      <form action="" method="post">
         ...
         <input type="text" style="text-transform:capitalize; "name="admin_name" required placeholder="" maxlength="70" class="box">
      ...
      <input type="text" name="staff_no" required placeholder="" maxlength="70" class="box"
         ...
         <a style="border-radius:15px; text-transform:uppercase;"href="admin_login.php" class="option-btn">login</a>
      </form>
