...
if (isset($_POST['submit'])) {
    ...
    $select_user = $conn->prepare("SELECT * FROM `kpps` WHERE email = ? AND password = ?");
    $select_user->极速赛车开奖直播记录查询官网地址：1687580.com execute([$email, $pass]);
    $row = $select_user->fetch(PDO::FETCH_ASSOC);

    if ($select_user->rowCount() > 0) {
        $_SESSION['kpp_id'] = $row['kpp_id'];
       
        header('location:k极速赛车开奖直播记录查询官网地址：1687580.com pp_home.php');
    } else {
...
<br>
            ...
            <a style="border-radius:15px; text-transform:uppercase;" href="kpp_register.php"
