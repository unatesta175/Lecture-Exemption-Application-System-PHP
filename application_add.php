...
if (isset($_POST['submit'])) {
    $reason = htmlspecialchars(strip_tags(trim($_POST['reason'])), ENT_QUOTES, 'UTF-8');
    $startDate = htmlspecialchars(strip_tags(trim($_POST['startDate'])), ENT_QUOTES, 'UTF-8');
    $endDate = htmlspecialchars(strip_tags(trim($_POST['endDate'])), ENT_QUOTES, 'UTF-8');
    $numDay = (abs(strtotime($endDate) - strtotime($startDate)) / 86400) + 1;
    $today = date("Y-m-d");
    
    if ($reason == "Lain-lain") {
        $reason = htmlspecialchars(strip_tags(trim($_POST['otherReason'])), ENT_QUOTES, 'UTF-8');
    }
    ...
    if ($startDate >= $sevenDaysbeforeStartDate) {
        ...
        if (isset($_FILES["supportDoc"]) && $_FILES["supportDoc"]["error"] == 0) {
            $allowed_ext = "pdf";
            $file_ext = strtolower(pathinfo($_FILES["supportDoc"]["name"], PATHINFO_EXTENSION));
            ...
            $supportDoc = rand(1000, 10000) . "-" . basename($_FILES["supportDoc"]["name"]);
            $tsupportDoc = $_FILES["supportDoc"]["tmp_name"];
            $uploads_supportDoc = 'supportDocfile';
            
            if (!move_uploaded_file($tsupportDoc, $uploads_supportDoc . '/' . $supportDoc)) {
                ?>
                <script>
                    alert('Gagal memuat naik dokumen sokongan!');
                </script>
                <?php
                exit();
            }
...
<br>
                                ...
                                <div class="">
                                    ...
                                    <input type="date" id="tarikhMula" required placeholder="" name="startDate"
                                        placeholder="Isi tarikh mula" class="form-control mb-2 bigger" min="<?php echo date('Y-m-d', strtotime('+7 days')); ?>">
