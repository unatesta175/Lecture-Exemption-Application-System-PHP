...
if (isset($_POST['support'])) {
    ...
    $support_status = 'Diluluskan';
    date_default_timezone_set('Asia/Kuala_Lumpur');
    $currentDateTime = date('Y-m-d H:i:s');
    $date_supported = $currentDateTime;
    $application_id = $_GET['application_id'];


    $update = $conn->prepare("UPDATE `applications` SET application_stat = ? ,support_status = ? , date_supported = ?  WHERE application_id = ?");
    $update->execute(['Selesai',$support_status,$date_supported, $application_id]);
         ...
         if ($resultx->rowCount() > 0) {
            ...
            $subject = 'Pemakluman Bahawasanya Permohonan Pengecualian Kuliah Oleh Pelajar Telah Berjaya dan Telah Diluluskan';
            $messagess = 'Assalammualaikum dan salam sejahtera ,'.$student_name.' .Sila layari laman web uitm dengan klik link <a href="http://10.90.72.114/eleas.uitm.edu.my/user_login.php">DISINI</a> untuk menyemak permohonan anda yang telah selesai';
            ...
            if($mail->Send())        //Send an Email. Return true on success or false on error
                  {
                      ...
                      <strong>Permohonan telah selesai divalidasi!</strong> Emel telah dihantar kepada pelajar sebagai makluman permohonan mereka berjaya dan telah Diluluskan.
                  ...
                  else
                  {
                      ...
                      echo "<div style='font-size:1.5em; margin-bottom:0px;'class='alert alert-success alert-dismissible fade show' role='alert'>
                      <strong>Permohonan telah selesai divalidasi!</strong> .
                      <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                    </div>
                    ";
                  }
...
} else if (isset($_POST['unsupport'])) {


    $support_status = 'Tidak Diluluskan';
    date_default_timezone_set('Asia/Kuala_Lumpur');
    $currentDateTime = date('Y-m-d H:i:s');
    $date_supported = $currentDateTime;
    $application_id = $_GET['application_id'];
...
if ($resultx->rowCount() > 0) {
   ...
   $subject = 'Pemakluman Bahawasanya Permohonan Pengecualian Kuliah Oleh Pelajar Gagal';
    $messagess = 'Assalammualaikum dan salam sejahtera ,'.$student_name.' .Sila layari laman web uitm dengan klik link <a href="http://10.90.72.114/eleas.uitm.edu.my/user_login.php">DISINI</a> untuk menyemak permohonan anda yang telah selesai';
   ...
   if($mail->Send())        //Send an Email. Return true on success or false on error
         {
             echo "<div style='font-size:1.5em; margin-bottom:0px;'class='alert alert-success alert-dismissible fade show' role='alert'>
             <strong>Permohonan telah selesai divalidasi!</strong> Emel telah dihantar kepada pelajar sebagai makluman permohonan mereka gagal.
             <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
           </div>
           ";
         }   
...
$no=1;
    ...
    name="support" onclick="return confirm(`Adakah anda pasti untuk "luluskan" permohonan pengecualian bagi kelas/kuliah ini?`);">
    ...
    name="unsupport" onclick="return confirm(`Adakah anda pasti untuk "Tidak luluskan" permohonan pengecualian bagi kelas/kuliah ini?`);">
';
                                                            } else {
                                                                echo ' <div style="display: flex; flex-wrap: nowrap;">


        <a href="kpp_support_menu.php" style=" text-transform: uppercase;border-radius: 15px; font-size: 14px; padding: 5px 10px; display: flex; align-items: center; justify-content: center; height: 30px; width: 100%;"  class=" text-light btn me-1 underline">SELESAI
     </a>
