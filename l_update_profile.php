...
if (isset($_POST['submit'])) {
   ...
   $update = $conn->prepare("UPDATE `lecturers` SET lecturer_name = ?, staff_no = ?, email = ?, work_phone_no= ? WHERE lecturer_id = ?");
   $update->execute([$lecturer_name , $staff_no , $email , $work_phone_no , $lecturer_id]);
   ...
   if (!isset($_SESSION['lecturer_id'])) {
      ...
      <section class="form-container">
         ...
         <form action="" method="post">
            ...
            $result = $conn->prepare("SELECT * FROM `lecturers` WHERE lecturer_id=?");
            ...
            <input type="text" name="lecturer_name" required placeholder="Isi nama penuh anda" maxlength="100" class="box"
               value="<?php echo $lecturer_name; ?>">
